...
if [ "$program" == "jmavsim" ] && [ ! -n "$no_sim" ]; then
	...
	"$src_path"/Tools/jmavsim_run.sh -i localhost -p 14577 -r 250 &
	SIM_PID=`echo $!`
...
if [[ ${model} == test_* ]] || [[ ${model} == *_generated ]]; then
	...
	sitl_command="\"$sitl_bin\" $no_pxh \"$src_path\"/ROMFS/px4fmu_common -s etc/init.d-posix/rcS-hil-main -t \"$src_path\"/test_data"
fi
